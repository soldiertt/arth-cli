...
export class CategoryRestService {
  ...
  findCategory(categoryName: string): Observable<Category> {
    return this.http.get<Category>(this.BASE_URL + "/category/name/" + categoryName).map(this._mapJsonToCategory);
  }

  listAll(): Observable<Category[]> {
    return this.http.get<Category[]>(this.BASE_URL + "/category/all").map(this._mapJsonArrayToCategoryArray);
  }

  listAllRoots(): Observable<Category[]> {
    return this.http.get<Category[]>(this.BASE_URL + "/category").map(this._mapJsonArrayToCategoryArray);
  }

  listSubCategories(categoryName: string): Observable<Category[]> {
    return this.http.get<Category[]>(this.BASE_URL + "/category/" + categoryName).map(this._mapJsonArrayToCategoryArray);
  }

  create(category: Category): Observable<Category> {
    return this.http.post<Category>(this.BASE_URL + "/category", Category.toDbObject(category)).map(this._mapJsonToCategory);
  }
